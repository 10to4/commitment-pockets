...
use super::{multiexp, multiexp2, EvaluationDomain, UniPolynomial};
use ark_ec::pairing::Pairing;
use ark_ec::AffineRepr;
use ark_ff::UniformRand;
use ark_std::rand::RngCore;
use ark_std::vec::Vec;
use ark_std::{ops::Add, ops::Mul, ops::Neg, ops::Sub, vec, Zero};
...
impl<E: Pairing> PedersenProof<E> {
    pub fn prove<R: RngCore>(
        params: &CaulkParameters<E>,
        v: E::ScalarField,
        rng: &mut R,
    ) -> Result<(Self, E::ScalarField), PocketError> {
    ...
    pub fn verify(&self, params: &CaulkParameters<E>) -> bool {
        ...
        let rhs = params.powers_of_g1[0]
            .mul(self.t1)
            .add(params.h.mul(self.t2))
            .into();
...
impl<E: Pairing> CaulkSingleProof<E> {
    pub fn prover<R: RngCore>(
        ...
        let mut t_commit = multiexp::<E>(
            &params.g1_vec(t_poly.degree() + 1).unwrap(),
            t_poly.deref().to_vec(),
        )
        .unwrap();
        t_commit = t_commit.add(params.h.mul(s)).into();

        let s_poly: UniPolynomial<E> =
            UniPolynomial::new(vec![a.mul(point).mul(s).sub(r), a.mul(s).neg()]);
    ...
    pub fn verify(&self, params: &CaulkParameters<E>, commit: &CaulkSinglePolyCommit<E>) -> bool {
        ...
        let lhs = E::pairing(
            (commit.commit.into_group() - &self.pi_ped.commit).into(),
            params.g2,
        );
        let rhs = E::multi_pairing(
            [&self.t_commit, &params.h],
            [&self.z_commit, &self.s_commit],
        );
