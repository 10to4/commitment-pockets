...
use super::{multiexp, multiexp2, UniPolynomial};
use ark_ec::pairing::Pairing;
use ark_ff::UniformRand;
use ark_std::rand::RngCore;
use ark_std::vec::Vec;
use ark_std::{ops::Mul, ops::Neg, vec, One, Zero};
...
impl<E: Pairing> MultiProofParameters<E> {
    pub fn new<R: RngCore>(max_degree: usize, rng: &mut R) -> Result<Self, PocketError> {
        ...
        let powers_of_g2 = (0..max_degree + 1)
            .map(|i| {
                if i != 0 {
                    cur = cur.mul(&tau);
                }
...
impl<E: Pairing> MultiProofPolyCommit2<E> {
    pub fn commit(
        ...
        let commit = multiexp2::<E>(
            &params.g2_vec(poly_degree + 1).unwrap(),
            poly.deref().to_vec(),
        )
        .unwrap();
        Ok(Self { commit })
    ...
    pub fn verify_eval(
        ...
        for (i, &point) in points.iter().enumerate() {
            ...
            for j in 0..coeffs.deref().len() {
                ipoly_coeffs[j] += coeffs.deref()[j].mul(self.values[i]);
            }
        }
        ...
        let rhs = E::multi_pairing(
            [&self.w, &ipoly_commit.commit],
            [&div_commit.commit, &params.powers_of_g2[0]],
        );
...
#[cfg(test)]
mod basic {
    use super::{MultiProof, MultiProofParameters, MultiProofPolyCommit, UniPolynomial};
