...
impl <E: Pairing> BasicPEDParameters<E> {
    pub fn new<R: RngCore>(max_degree :usize, rng: &mut R) -> Result<Self, PocketError> {
        ...
        let powers_of_g = (0..max_degree+1).map(|i|{
            if i != 0 {
                cur = cur.mul(&tau);
            }
            cur.into()
        ...
        let mut cur = E::G1::rand(rng);
        let powers_of_h = (0..max_degree+1).map(|i|{
            if i != 0 {
                cur = cur.mul(&tau);
            }
            cur.into()
...
impl <E: Pairing> BasicPEDPolyCommit<E>{
    ...
    pub fn commit<R: RngCore>(params: &BasicPEDParameters<E>, poly: &UniPolynomial<E>, rng: &mut R) -> Result<(Self, UniPolynomial<E>), PocketError>{
    ...
    pub fn open(&self, params: &BasicPEDParameters<E>, poly: &UniPolynomial<E>, phi_poly: &UniPolynomial<E>) -> Result<(UniPolynomial<E>, UniPolynomial<E>), PocketError>{
        let poly_degree = poly.degree();
    ...
    pub fn verify_poly(&self, params: &BasicPEDParameters<E>, poly: &UniPolynomial<E>, phi_poly: &UniPolynomial<E>) -> bool {
        let poly_degree = poly.degree();
        let phi_degree = phi_poly.degree();
        let param_degree = params.degree();
        assert!(phi_degree == poly_degree);
...
impl <E: Pairing> BasicPEDProof<E>{
    ...
    pub fn create_witness(params: &BasicPEDParameters<E>, poly: &UniPolynomial<E>, phi_poly: &UniPolynomial<E>, point: E::ScalarField) -> Result<Self, PocketError>{
        let poly = poly.clone();
    ...
    pub fn verify_eval(&self, params: &BasicPEDParameters<E>, commit: &BasicPEDPolyCommit<E>, point: E::ScalarField) -> bool {
        
        let lhs = E::pairing(&commit.commit, &params.g2);

        let rhs = E::multi_pairing([&self.w, &params.powers_of_g[0].mul(self.value).add(params.powers_of_h[0].mul(self.phi_value)).into()], [&(params.g2_tau.into_group() - &params.g2.mul(point).into()).into(), &params.g2]);
...
mod basic_ped {
    ...
    use ark_ff::UniformRand;
    use super::{BasicPEDParameters, BasicPEDPolyCommit, UniPolynomial, BasicPEDProof};
    ...
    fn test_basic_ped_poly_commit() {
        ...
        let (commit, phi_poly) = BasicPEDPolyCommit::<Bls12_381>::commit(&params, &poly, rng).unwrap();
        let challenge = Fr::rand(rng);
        let proof = BasicPEDProof::<Bls12_381>::create_witness(&params, &poly, &phi_poly, challenge).unwrap();
